            ...
            <el-row>
				...
				<el-col :span="8">
				    <el-form-item label="是否上架"
				        prop="status">
				        <el-radio 
				            label="1"  v-model="radio" @change="getVal()"
				            border>上架</el-radio>
				        <el-radio 
				            label="2"  v-model="radio"  @change="getVal()"
				            border>待上架</el-radio>
				    </el-form-item>
				</el-col>
            </el-row>
...
import { getAddress,addServ,getServ,editServ } from "@/api/serv"
...
export default {
    ...
    data() {
        return {
            ...
            formRules: {
                ...
                status: [
                    { required: true,  message: '非法数据', trigger: 'blur' }
                ],
				imgurl: [
    ...
    methods: {
		getVal()
		{
			this.formValue.status = this.radio
		}
		,
		_getData() {
		        getServ(this.$route.query).then(res => {
        ...
        handleRemove(file, fileList) {
            this.imgs.splice(inArray(this.imgs, file.response.data), 1)
            console.log('handleRemove', file, fileList)
			
        },
        //图片上传钩子
        uploadSuccess(res, file, fileList) {
            this.imgs.unshift(res.data)
        ...
        onSubmit() {
            this.$refs.fromInput.validate(valid => {
                if (!valid) {
                 ...
                 editServ(this.formValue).then(res => {
					 console.log(res)
                    this.$message({
                        message: res.msg,
                        type: 'success'
